...
def parse_metadata(metadata):
    ...
    if attributes := metadata.get("attributes"):
        ...
        if created := attributes.get("created_at"):
            output["dateCreated"] = datetime.datetime.strptime(created, "%Y-%m-%dT%H:%M:%S.%fZ").strftime("%Y-%m-%d")

        if updated := attributes.get("updated_at"):
            output["dateModified"] = datetime.datetime.strptime(updated, "%Y-%m-%dT%H:%M:%S.%fZ").strftime("%Y-%m-%d")
