...
def parse():
    ...
    mime_type = {
        ...
        "fasta.gz": "application/x-fasta",
        "tar": "application/x-tar",
        "tar.gz": "application/x-tar",
        "gtf": "application/x-gtf",
        "gtf.gz": "application/x-gtf",
        "html": "text/html",
        "sam": "application/x-sam",
        "bam": "application/x-bam",
        "zip": "application/zip",
        "vcf": "application/x-vcf",
        "vcf.gz": "application/x-vcf",
        "bed": "text/x-bed",
        "bed.gz": "text/x-bed",
    ...
    while True:
        data_count = 0
        url = (
            f"https://cavatica.sbgenomics.com/ns/amigo/api/v1/apps/explore?limit={limit}&offset={offset}&order_by=label"
        )
    ...
    for id in public_ids:
        app_meta_data = requests.get("https://cavatica.sbgenomics.com/ns/brood/v1/raw/" + id)
    ...
    for data in all_app_meta_data:
        ...
        if modified_on := data.get("sbg:modifiedOn"):
            output["dateModified"] = datetime.utcfromtimestamp(modified_on).strftime("%Y-%m-%d")
        if created_on := data.get("sbg:createdOn"):
            output["dateCreated"] = datetime.utcfromtimestamp(created_on).strftime("%Y-%m-%d")
