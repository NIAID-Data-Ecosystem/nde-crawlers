...
def record_generator():
    ...
    logging.info("[INFO] processing %s records...." %
                 len(json_records['records']))
    ...
    for _record_dict in json_records['records']:
        ...
        _record_dict['measurementTechnique'] = {
            'name': _record_dict['attributes'].pop('type')}
        _record_dict['sdPublisher'] = {
            'name': _record_dict['attributes'].pop('project_id')}
        _record_dict['creditText'] = _record_dict['attributes'].pop(
            'short_attribution')

        if _record_dict['attributes']['release_policy']:
            _record_dict['conditionOfAccess'] = _record_dict['attributes'].pop(
                'release_policy')
        ...
        if _record_dict['attributes']['version']:
            try:
                ...
                date_modified = datetime.datetime.strptime(
                    date_modified, '%Y-%m-%d').date().isoformat()
                _record_dict['dateModified'] = date_modified
            except:
                logging.debug("[INFO] BAD DATE FROM _record_dict['attributes']['version']: %s" %
                              _record_dict['attributes']['version'])

        # tablexs.Contacts
        _record_dict['author'] = [{'name': _dict.pop('contact_name'), "affiliation": {'name': str(
            _dict.pop("affiliation"))}} for _dict in _record_dict['tables']['Contacts']]
        # tables.GenomeHistory
        release_dates = [hit['release_date']
                         for hit in _record_dict['tables']['GenomeHistory']]
        # if multiple dates passed, keep the most recent date
        if release_dates:
            try:
                iso_list = [datetime.datetime.strptime(
                    d, '%Y-%m-%d').date().isoformat() for d in release_dates]
                date_updated = sorted(iso_list)[0]
                _record_dict['dateUpdated'] = date_updated
            except:
                logging.debug(
                    "[INFO] BAD DATE FROM _record_dict['tables']['Version']: %s" % release_dates)

        # tables.Version
        published_dates = [hit['version']
                           for hit in _record_dict['tables']['Version']]
        if published_dates:
            try:
                _iso_list = [datetime.datetime.strptime(
                    d, '%Y-%m-%d').date().isoformat() for d in published_dates]
        ...
        gene_counts = [hit['gene_count']
                       for hit in _record_dict['tables']['GeneTypeCounts']]
        gene_refs = [hit['gene_type']
                     for hit in _record_dict['tables']['GeneTypeCounts']]
        if gene_refs:
