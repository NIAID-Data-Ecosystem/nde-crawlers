...
class MalariaGenSpider(scrapy.Spider):
    ...
    def parse_dataset(self, response):
        ...
        release_text = response.xpath(
            '//div[@class="hero-summary standard white"]/p[contains(text(), "Released on")]/text()'
        ).get()
        ...
        author_url = response.urljoin(
            response.xpath('//blockquote[contains(.//h3, "Data package contact")]//a/@href').get(default=None)
        )
        ...
        keywords = [
            keyword for keyword in keywords if not any(keyword.startswith(bad_keyword) for bad_keyword in bad_keywords)
        ]

        # Extracting the correct citation URL
        citation_url = response.xpath(
            '//blockquote[contains(.//h3, "Citations")]//a[@data-wpel-link="external"]/@href'
        ).get(default=None)

        # Extracting 'isPartOf' data correctly, now capturing all text within the <a> tag, including nested tags
        project_name_parts = response.css(
            ".hero-info.white a *::text"
        ).getall()  # Extracts all text components including from within <i>
        ...
        open_urls = [
            "https://www.malariagen.net/project/pf3k/",
            "https://www.malariagen.net/project/p-falciparum-community-project/",
        ]
