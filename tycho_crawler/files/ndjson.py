...
class NDJsonWriterPipeline:
    ...
    def from_crawler(cls, crawler: scrapy.crawler.Crawler):
        ...
        ndjson_writer = cls()
        crawler.signals.connect(receiver=ndjson_writer.invalidate_data, signal=scrapy.signals.spider_error)
    ...
    def open_spider(self, spider: scrapy.spiders.Spider):
        ...
        self.release_string = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        dirname = os.path.join("/data", f"{spider.name}_crawled")
    ...
    def close_spider(self, spider: scrapy.spiders.Spider):
        ...
        if not self._valid:
            spider.logger.warning("Errors occurred while running, so not saving potentially corrupt data.")
        ...
        try:
            ...
            errors = []
            for fn in [self.tmp_filename, self.final_data_filename, self.rl_tmp_filename, self.release_filename]:
                try:
                    os.unlink(fn)
