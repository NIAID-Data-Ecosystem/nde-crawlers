...
for _ in ["datasources", "plugins", "dataupload", "diff", "logs", "release", "cache", "run", "esbackup"]:
...
if not os.path.isfile(ssh_key_file):
    if os.path.exists(ssh_key_file):
        raise FileExistsError(f"{ssh_key_file} exists but is not a regular file")
    ...
    with open(ssh_key_file, "wb") as f:
        f.write(
            privkey.private_bytes(crypto_ser.Encoding.PEM, crypto_ser.PrivateFormat.OpenSSH, crypto_ser.NoEncryption())
        )
