...
DB_PATH = '/cache/funding_lookup/funding_lookup.db'
...
def create_sqlite_db(DB_PATH):
    ...
    c = conn.cursor()
    c.execute('''CREATE TABLE funding_lookup
...
def update_sqlite_db(funding_id, new_funding):
    ...
    """
    logger.info(
        f"Updating funding information for {funding_id} in sqlite database.")
    ...
    c.execute("INSERT INTO funding_lookup VALUES (?, ?)",
...
def sqlite_lookup(funding_id):
    ...
    """
    logger.info(
        f"Looking up funding information for {funding_id} in sqlite database.")
    funding_id = funding_id.replace(' ', '').lower()
    ...
    if result:
        logger.info(
            f'Successfully found funding information for {funding_id}.')
...
def update_funding(funding_id):
    ...
    if len(funding_id) < 5:
        logger.info(
            f'INVALID FUNDING ID FOR {funding_id}')
        return None
    ...
    while True:
        ...
        params = {
            "criteria":
            {
                "project_nums": [funding_id]
            },
            "include_fields": [
                "ProjectTitle", "ApplId", "SubprojectId", "FiscalYear", "Organization", "ProjectNum", "OrgCountry",
                "ProjectNumSplit", "PrincipalInvestigators", "AllText", "FullStudySection",
                "ProjectStartDate", "ProjectEndDate", "AbstractText", "Terms", "FullFoa", "ProgramOfficers", "AwardAmount"
            ],
            "offset": offset,
            "limit": 500,
        ...
        response = requests.post(
            "https://api.reporter.nih.gov/v2/projects/search", json=params)
        try:
        ...
        if funding_data == ['Invalid project number']:
            logger.info(
                f'INVALID PROJECT NUMBER FOR {funding_id}')
            return None
        elif funding_data['meta']['total'] == 0:
            logger.info(
                f'NO RESULTS FOUND FOR {funding_id}')
            return None
        else:
    ...
    logger.info(
        f'FOUND {len(data)} RESULTS FOR {funding_id}')
    ...
    if len(parent) == 0:
        ...
        logger.info(
            f'MULTIPLE PARENT PROJECTS FOUND FOR {funding_id}')
        ...
        largest_amount_parents = [
            i for i in parent if i['award_amount'] == largest_amount]
        # if there is only one parent with the largest amount, use it
        if len(largest_amount_parents) == 1:
            logger.info(
                f'USING LARGEST AWARD AMOUNT PARENT FOR {funding_id}')
        ...
        # if there are more than one parent with the largest amount, use the one application_type is 1
            ...
            logger.info(
                f'MULTIPLE PARENTS WITH LARGEST AMOUNT FOUND FOR {funding_id}')
            for large_parent in largest_amount_parents:
                if large_parent['project_num_split'] is not None and large_parent['project_num_split']['appl_type_code'] == '1':
                    logger.info(
                        f'USING PARENT WITH APPLICATION TYPE 1 FOR {funding_id}')
                    return build_funding_dict(large_parent)
            logger.info(
                f'NO PARENT WITH APPLICATION TYPE 1 FOUND FOR {funding_id}, USING FIRST INDEX')
            return build_funding_dict(largest_amount_parents[0])
...
def build_funding_dict(funding_info):
    ...
    ic_lookup = {'CC': 'Clinical Center', 'RG': 'Center for Scientific Review', 'CIT': 'Center for Information Technology', 'TW': 'John E. Fogarty International Center', 'TR': 'National Center for Advancing Translational Sciences (NCATS)', 'AT': 'National Center for Complementary and Integrative Health', 'CA': 'National Cancer Institute', 'RR': 'National Center for Research Resources (dissolved 12/2011)', 'EY': 'National Eye Institute', 'HG': 'National Human Genome Research Institute', 'HL': 'National Heart, Lung, and Blood Institute', 'AG': 'National Institute on Aging', 'AA': 'National Institute on Alcohol Abuse and Alcoholism', 'AI': 'National Institute of Allergy and Infectious Diseases', 'AR': 'National Institute of Arthritis and Musculoskeletal and Skin Diseases', 'EB': 'National Institute of Biomedical Imaging and Bioengineering',
                 'HD': 'Eunice Kennedy Shriver National Institute of Child Health and Human Development', 'DA': 'National Institute on Drug Abuse', 'DC': 'National Institute on Deafness and Other Communication Disorders', 'DE': 'National Institute of Dental and Craniofacial Research', 'DK': 'National Institute of Diabetes and Digestive and Kidney Diseases', 'ES': 'National Institute of Environmental Health Sciences', 'GM': 'National Institute of General Medical Sciences', 'MH': 'National Institute of Mental Health', 'MD': 'National Institute on Minority Health and Health Disparities', 'NS': 'National Institute of Neurological Disorders and Stroke', 'NR': 'National Institute of Nursing Research', 'LM': 'National Library of Medicine', 'OD': 'Office of the Director'}
