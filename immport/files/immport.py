...
def get_ids():
    ...
    ids = []
    while True:
        immport_search_payload = {'pageSize': page_size,
                                  'fromRecord': from_record, 'sortFieldDirection': 'desc'}
        try:
            j = requests.get(base_url, timeout=5,
                             params=immport_search_payload,).json()
...
def map_schema_json(schema_json):
    ...
    if author := schema_json.pop('creator', None):
        for data in author:
            for key, value in data.items():
                data[key] = {
                    "name": value} if key == "affiliation" else value
    ...
    if date := schema_json.pop('curationDate', None):
        date = datetime.datetime.strptime(
            date, "%m/%d/%Y")  # mm/dd/YYYY is my guess
        if distribution := schema_json.pop('distribution', None):
            schema_json['distribution'] = _set_single_element_or_all_in_list(
    ...
    if condition_studied := details_json.get('conditionStudied'):
        ...
        if len(health_conditions):
            ...
            health_conditions = list(
                {v['name']: v for v in health_conditions}.values())
    ...
    if protocols := protocol_json.get('protocols'):
        ...
        for protocol in protocols:
            ...
            if bool(is_based_on_dict):
                is_based_on_dict[
                    'url'] = f'https://www.immport.org/browser/?path={immport_id}'
...
def parse():
    ...
    for immport_id in ids:
        count += 1
        logger.info(
            f'Processing ImmPort ID: {immport_id} ({count}/{len(ids)})')
        ...
        try:
            ...
            existing_health_conditions = schema_json.get(
                'healthCondition', None)
            existing_measurement_technique = schema_json.get(
                'measurementTechnique', None)

        except requests.exceptions.Timeout:
            # catch the rather harmless exception, and do nothing
            logger.info(
                f'Timeout searching ImmPort ID schema_url: {immport_id}')
        ...
        try:
            ...
            details_json = map_details_json(
                details_json, existing_health_conditions, existing_measurement_technique)
        except requests.exceptions.Timeout:
            # catch the rather harmless exception, and do nothing
            logger.info(
                f'Timeout searching ImmPort ID details_url: {immport_id}')
        ...
        try:
            ...
            # catch the rather harmless exception, and do nothing
            logger.info(
                f'Timeout searching ImmPort ID protocol_url: {immport_id}')
