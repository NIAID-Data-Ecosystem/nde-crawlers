...
dirname = os.path.join("/data", "pgs_catalog_crawled")
...
is_parsed = False
try:
    for doc in pgs_catalog.parse():
        fd.write(orjson.dumps(doc) + b"\n")
    is_parsed = True
except Exception:
...
if is_parsed:
    try:
        ...
        errors = []
        for filename in [tmp_filename, final_data_filename, rl_tmp_filename, release_filename]:
            try:
                os.unlink(filename)
            except FileNotFoundError:
                pass
            except Exception as exc:
                logger.critical("Failed to remove corrupt file %s", filename)
                errors.append(exc)
        if errors:
