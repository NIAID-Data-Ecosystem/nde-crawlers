...
def get_ids():
    ...
    payload = {"query": {"bool": {"must": [{"bool": {"must_not": [{"exists": {"field": "next_revision_uuid"}}, {"exists": {"field": "sub_status"}}]}}, {"bool": {"must_not": [
        {"exists": {"field": "next_revision_uuid"}}, {"exists": {"field": "sub_status"}}]}}]}}, "post_filter": {"term": {"entity_type.keyword": "Dataset"}}, "_source": False, "size": 10000}
...
def parse():
    ...
    for metadata in datasets:
        ...
        if created_timestamp := metadata.get('created_timestamp'):
            output['dateCreated'] = datetime.datetime.utcfromtimestamp(
                created_timestamp / 1000).strftime('%Y-%m-%d')
        ...
        if last_modified_timestamp := metadata.get('last_modified_timestamp'):
            output['dateModified'] = datetime.datetime.utcfromtimestamp(
                last_modified_timestamp / 1000).strftime('%Y-%m-%d')

        if published_timestamp := metadata.get('published_timestamp'):
            output['datePublished'] = datetime.datetime.utcfromtimestamp(
                published_timestamp / 1000).strftime('%Y-%m-%d')
            ...
            if nested_metadata := dataset_metadata.get('metadata'):
                ...
                if section_prep_protocols_io_doi := nested_metadata.get('section_prep_protocols_io_doi'):
                    if 'isBasedOn' in output:
                        ...
                        output['isBasedOn'] = {
                            'doi': section_prep_protocols_io_doi}
