...
class SRAdb(BASEdb):
    ...
    def sra_metadata(
        ...
        for single_acc in acc:
            ...
            if in_acc_type not in self.valid_in_acc_type and not acc_is_searchstr:
                raise ValueError(
                    "{} not a valid input type".format(in_acc_type))
            if acc_is_searchstr:
                in_type = "study"
            else:
    ...
    def search_by_expt_id(self, srx):
        ...
        if "GSM" in srx:
            results = self.cursor.execute(
                'select * from EXPERIMENT where experiment_alias = "{}"'.format(
                    srx)
            ).fetchall()
        else:
            results = self.cursor.execute(
                'select * from EXPERIMENT where experiment_accession = "{}"'.format(
                    srx)
            ).fetchall()
    ...
    def _format_dataframe_for_download(self, df, url_column, use_ascp):
        ...
        accession_columns = ["study_accession",
        ...
        if url_column and url_column in df_columns:
            ...
            for i in range(1, run_count):
                ...
                run_df = run_df.rename(
                    columns={f"run_{i}_accession": "run_accession"})
            ...
            if run_count == 1:
                ...
                df["recommended_url"] = df.apply(
                    lambda row: self._select_best_url(
                        matched_cols, row, use_ascp),
                    axis=1,
                ).tolist()
    ...
    def download(
        ...
        if not use_ascp:
            ...
            df["filesize"] = df["filesize"].apply(
                lambda x: millify(x)).tolist()
