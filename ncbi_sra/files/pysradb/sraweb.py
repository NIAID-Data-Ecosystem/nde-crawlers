...
class SRAweb(SRAdb):
    ...
    def fetch_ena_fastq(self, srp):
        ...
        request = requests.get(self.ena_fastq_search_url,
        ...
        for line in request_text.split("\n"):
            ...
            ftp_url = url.replace("ftp.sra.ebi.ac.uk/",
        ...
        if ";" in request_text:
            ...
            return pd.DataFrame(
                urls, columns=["run_accession",
                               "ena_fastq_http", "ena_fastq_ftp"]
            )
        ...
        **kwargs
        ...
        for uid, run_json in runs_json.items():
            ...
            if isinstance(exp_platform, OrderedDict):
                exp_platform_model = exp_platform.get(
                    "@instrument_model", pd.NA)
            ...
            library_construction_protocol = False
            if 'LIBRARY_CONSTRUCTION_PROTOCOL' in exp_library_descriptor:
                library_construction_protocol = exp_library_descriptor["LIBRARY_CONSTRUCTION_PROTOCOL"]
            if isinstance(library_construction_protocol, dict):
                library_construction_protocol = library_construction_protocol["#text"]
            ...
            for run_record in runs:
                ...
                if library_construction_protocol is not False:
                    experiment_record['library_construction_protocol'] = library_construction_protocol
        ...
        for record in efetch_result:
            ...
            for run_set in run_sets:
                ...
                if study_abstract is not False:
                    detailed_record["study_abstract"] = study_abstract
                ...
                # SUBMITTER
                submitter_info = exp_json['Submitter']
                if submitter_info := exp_json.get('Submitter'):
                    if submitter_center := submitter_info.get('@center_name'):
                        detailed_record['submitter_center'] = submitter_center
                    if contact_name := submitter_info.get('@contact_name'):
                        detailed_record['contact_name'] = contact_name
                    if lab_name := submitter_info.get('@lab_name'):
                        detailed_record['lab_name'] = lab_name
                    ...
                    for sra_file in sra_files:
                        ...
                        if "@filename" in sra_file:
                            if sra_file["@filename"] == run_set["@accession"]:
                                detailed_record["sra_url"] = sra_file.get(
                                    "@url", pd.NA)
                ...
                detailed_record["experiment_alias"] = expt_ref.get(
                    "@refname", "")
        ...
        for uid in uids:
            ...
            if record["extrelations"]:
                ...
                for extrelation in extrelations:
                    ...
                    assert len(values) == 3
                    record[extrelation["relationtype"]
                           ] = extrelation["targetobject"]
    ...
    def gsm_to_srs(self, gsm, **kwargs):
        ...
        gsm_df = gsm_df[gsm_df.entrytype == "GSM"].rename(
            columns={"SRA": "experiment_accession",
                     "accession": "experiment_alias"}
        )
