...
class NDEDatabase:
    ...
    def retrieve_last_record(self):
        """Helper method for update_cache. Gets the last record from cache table."""
        ...
        # checks last record
        c.execute(
            "SELECT * FROM cache WHERE _id=(SELECT max(_id) FROM cache)")

        last_record = c.fetchall()
        last_record_id = last_record[0][0]
        con.close()
    ...
    def retreive_cache(self):
        ...
        for record in c:
            ...
            if count % 10000 == 0:
                logger.info(
                    "Retrieving and parsing cache. Parsed %s records", count)
            yield record
