...
# Output goes to /data/usidnet_crawled/ so the biothings-hub dumper can locate
# data.ndjson at the conventional path (the spider class is named
# "usidnet_spider", so we cannot rely on spider.name here).
_OUTPUT_DIRNAME = "usidnet_crawled"
...
class NDJsonWriterPipeline:
    ...
    def from_crawler(cls, crawler: scrapy.crawler.Crawler):
        ...
        ndjson_writer = cls()
        ndjson_writer.crawler = crawler
        crawler.signals.connect(
            receiver=ndjson_writer.invalidate_data, signal=scrapy.signals.spider_error
        )
    ...
    def open_spider(self):
        ...
        self.release_string = datetime.datetime.now(datetime.timezone.utc).strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        )
        dirname = os.path.join("/data", _OUTPUT_DIRNAME)
        os.makedirs(dirname, exist_ok=True)
        self.release_filename = os.path.join(dirname, "release.txt")
        # so that updates are as atomic as possible, using rename
        self.final_data_filename = os.path.join(dirname, "data.ndjson")
        self.tmp_filename = (
            f"{self.final_data_filename}.{platform.node()}.{os.getpid()}"
        )
        self.rl_tmp_filename = (
            f"{self.release_filename}.{platform.node()}.{os.getpid()}"
        )
        ...
        try:
            ...
            for fn in [
                ...
                try:
                    ...
                    spider.logger.critical(f"Failed to remove corrupt file {fn}")
                    errors.append(e)
            if errors:
                # re-raise all errors
